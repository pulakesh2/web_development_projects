...
// create a peek operation which return the value of a position -->
int peek(struct stack*ptr,int i){
    // see that the no user type is invalid or not-->
    if (ptr->top - i+1<0)
    {
...
int main(){
    ...
    printf("Enter the index no to show the element\n");
    scanf("%d",&search);


    //call the peek function-->
    printf("the %d position --> %d",search,peek(s1,search));   
    return 0;
}