...
struct node*insertion_before_an_element(struct node*head,int value,int data){
...
int main(){
    ...
    // take the element so that the node will be inserted before that element-->
    printf("enter the value before which element the node will insert\n");
    scanf("%d",&value);



    // call the insertion function-->
    first = insertion_before_an_element(first,value,data);
