...
struct node*insertion_after_a_element(struct node*head,int value,int data){
...
int main(){
    ...
    // take the element so that the node will be inserted after that element-->
    printf("enter the value after which element the node will insert\n");
    scanf("%d",&value);



    // call the insertion function-->
    first = insertion_after_a_element(first,value,data);
