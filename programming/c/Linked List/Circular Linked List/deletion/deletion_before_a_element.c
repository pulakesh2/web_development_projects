...
struct node*deletion_before_an_element(struct node*head,int value){
    struct node*ptr = (struct node*)malloc(sizeof(struct node));
    struct node*preptr = (struct node*)malloc(sizeof(struct node));
    struct node*new_node = (struct node*)malloc(sizeof(struct node));
    ptr = head;
    preptr = ptr->next;
    while (preptr->next->data!=value)
    {
        ptr = ptr->next;
        preptr = preptr->next;
    }
...
int main(){
    ...
    // take the element so that the node will be deleted before that element-->
    printf("\nenter the value before which element the node will insert\n");
    scanf("%d",&value);



    // call the deletion function-->
    first = deletion_before_an_element(first,value);
