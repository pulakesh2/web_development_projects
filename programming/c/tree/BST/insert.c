...
void insert(struct tree*root,int value){
    struct tree*prev = (struct tree*)malloc(sizeof(struct tree));
     while (root!=NULL)
     {
         ...
         if (value==root->data)
         {
             printf("already exist in the tree\n");
             return;
...
int main()
{
    ...
    printf("after insertion the tree in inorder-->");
    inOrder(t1);
